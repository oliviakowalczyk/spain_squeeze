...
for i in "${sub[@]}"; do
	for j in "ses-A" "ses-B"; do
        for l in "run-1" "run-2"; do
			...
			data=chop_MCnofilt_tmean.nii.gz

			echo $i $j $l 
			
			#this is the SCT 6.3 version of deepseg
            #sct_deepseg_sc -i ${data} -c t2s
            
            #updated to reflect the EPI-specific segmentation algorithm
            sct_deepseg -i ${data} -c t2star -task seg_sc_epi
