...
for i in "${sub[@]}"; do
	for j in "ses-A" "ses-B"; do
        for l in "run-1" "run-2"; do
			...
			sct_deepseg spinalcord -i ${anat}.nii.gz -qc ${dir}/qc -o ${anat}_seg.nii.gz
			
			echo "Segmenting rootlets"
			sct_deepseg rootlets -i ${anat}.nii.gz -qc ${dir}/qc -o ${anat}_rootlets.nii.gz

			echo "Create centre-of-mass of rootlets for visual inspection"
			sct_label_utils -i ${anat}_rootlets.nii.gz -cubic-to-point -o ${anat}_rootlets_mid.nii.gz
			sct_label_utils -i ${anat}_seg.nii.gz -project-centerline ${anat}_rootlets_mid.nii.gz -o ${anat}_rootlets_mid_center.nii.gz
			sct_qc -i ${anat}.nii.gz -s ${anat}_rootlets_mid_center.nii.gz -p sct_label_utils -qc ${dir}/qc
			...
			echo "Registering T2w image to PAM50 template using rootlets"
			sct_register_to_template -i ${anat}.nii.gz -s ${anat}_seg.nii.gz -lrootlet ${anat}_rootlets.nii.gz -ofolder reg_rootlets -qc ${dir}/qc
			
			echo "Creating new warp files"
			sct_concat_transfo \
			-d ${sct_dir}/data/PAM50/template/PAM50_t2s.nii.gz \
			-w warp_${func}_tmean2${anat}.nii.gz reg_rootlets/warp_anat2template.nii.gz \
			-o reg_rootlets/warp_${func}_tmean2PAM50.nii.gz

			sct_concat_transfo \
			-d ${func}_tmean.nii.gz \
			-w reg_rootlets/warp_template2anat.nii.gz warp_${anat}2${func}_tmean.nii.gz \
			-o reg_rootlets/warp_PAM502${func}_tmean.nii.gz
			...
			-initwarp reg_rootlets/warp_PAM502${func}_tmean.nii.gz \
			-initwarpinv reg_rootlets/warp_${func}_tmean2PAM50.nii.gz \
			-ofolder reg_rootlets 
