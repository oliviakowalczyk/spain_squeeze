	...
	for j in "ses-A" "ses-B"; do
        for l in "run-1" "run-2"; do
            ...
            for m in "cope1" "cope2"; do
			    ...
			    sct_apply_transfo -i ${m}.nii.gz -d ${sct_dir}/data/PAM50/template/PAM50_t2s.nii.gz -w ${dir}/derivatives/preprocessing/${i}/${j}/${l}/reg_rootlets/warp_chop_MCnofilt_tmean2PAM50_t2s.nii.gz -x linear
			done
        done
    done
done

# Loop over all subjects for parametric modulation analysis
for i in "${sub[@]}"; do
	for j in "ses-A" "ses-B"; do
        for l in "run-1" "run-2"; do
			
            cd $dir/derivatives/subject_level/${i}_${j}_${l}_first_trial_censored_paramod.feat/stats

            echo "Processing " ${i} ${j} ${l}
            for m in "cope1" "cope2" "cope3"; do
